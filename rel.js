...
async function startGenericSession({ numero, isPrincipale = false, sessionId = null }) {
  ...
  try {
    ...
    sock.recup_msg = (options = {}) => recup_msg({ rel: sock, ...options });
...
async function startPrincipalSession() {
  ...
  if (!(sessionId && sessionId.startsWith("rel-MD_") && sessionId.endsWith("_SESSION-ID"))) {
